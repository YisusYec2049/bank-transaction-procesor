...
Esquema de salida normalizado (10 columnas, índices 0-9):
  [0] identification      ← referencia_1 sin ceros iniciales
  [1] payment_date        ← DD-MM-YYYY  (ISO YYYY-MM-DD al hacer upsert)
  ...
  [9] matching_key        ← {DD/MM/YYYY}_{ref1}_{valor}
"""
...
# Tipos abiertos: después del keyword viene un nombre variable que se conserva.
_ABIERTAS = [
...
# Ordenadas de más larga a más corta para match greedy (igual que n8n).
...
def _parse_valor(s: str) -> float | None:
    """
    Detecta formato europeo (1.435.500,00) vs americano (300,000.00) POR CUENTA.
...
def _valor_str(v) -> str:
    ...
    Necesario para que matching_key sea idéntico al generado por n8n.
      500000.0  → "500000"
      435.5     → "435.5"
      435.53    → "435.53"
    """
...
def _norm_valor_str(raw) -> str:
    """Normaliza un valor leído desde Sheets (puede ser string o número) a _valor_str."""
    try:
        return _valor_str(float(str(raw).replace(',', '').strip()))
...
def _extraer_refs(texto: str) -> tuple[str, str, str]:
    ...
    if not matches:
        # Si el texto empieza con una sola palabra todo-mayúsculas (sucursal
        # pegada por extract_text), se descarta ese prefijo para obtener el
        # nombre real de referencia. Ej: 'TOBERIN KAREN GOMEZ' → 'KAREN GOMEZ'
...
def _build_sucursal_lookup(pdf_bytes: io.BytesIO) -> dict:
    """
    Construye {(fecha_DD/MM/YYYY, valor_float): sucursal_str} usando posiciones
    X de palabras. El sucursal siempre aparece en x0 ∈ [240, 329) en este PDF.
    """
    lookup: dict = {}
...
def parse_pdf(pdf_bytes: io.BytesIO) -> list[dict]:
    """
    Extrae transacciones del extracto PDF de Bancolombia 2576.
    Retorna lista de dicts con campos raw (antes de normalizar al esquema estándar).
    """
    ...
    while i < len(lineas):
        ...
        if m_date:
            ...
            fecha = f'{dia}/{mes}/{anio}'  # DD/MM/YYYY
            resto = m_date.group(2).strip()

            # Continuación: si no hay valor al final, pegar línea siguiente.
            ...
            # Sucursal real desde posiciones X (más fiable que el texto plano)
            sucursal = suc_lookup.get((fecha, valor), '')
            # Para tipos ABIERTOS el sucursal queda pegado al final del nombre;
            # se elimina para que transaction_code_1 quede limpio.
            if sucursal:
                suf = ' ' + sucursal
    ...
    # Filtrar descripciones no deseadas y valores negativos.
    validas = [
        f for f in filas
    ...
    # Dedup interno por fecha+descripcion+valor (mismo criterio que n8n).
    seen, resultado = set(), []
    for f in validas:
    ...
    log.info('parse_pdf: %d filas brutas → %d tras filtros y dedup', len(filas), len(resultado))
    return resultado


def normalize(raw_rows: list[dict]) -> list[list]:
    """
    Convierte filas raw al esquema estándar de 11 columnas.
    matching_key = {DD/MM/YYYY}_{ref1_sin_ceros}_{valor_js_format}
    payment_date se guarda internamente como DD-MM-YYYY.
    """
    result = []
    for row in raw_rows:
        ...
        valor        = row['valor']
        matching_key = f'{fecha}_{ref1}_{_valor_str(valor)}'
        payment_date = fecha.replace('/', '-')            # DD-MM-YYYY

        result.append([
            ref1,               # [0]  identification
            payment_date,       # [1]  payment_date
            row['descripcion'], # [2]  transaction_code_1
            row['sucursal'],    # [3]  transaction_code_2
            ref1,               # [4]  email (= identification en esta fuente)
            'BANCOLOMBIA',      # [5]  payment_method
            '',                 # [6]  program
            '',                 # [7]  phone
            valor,              # [8]  payment_amount
...
def cheque_logic(
    ...
    """
    Clasifica filas en normales y cheques, y aplica lógica de conciliación.

    pendientes_raw: valores crudos de 'Hoja 1'!A1:M del sheet CHEQUES_PENDIENTES
                   (primera fila puede ser cabecera o datos según origen).

    Retorna:
        normales          – filas sin CHEQUE en la descripción
        pendientes_nuevos – cheques nuevos (→ escribir con estado PENDIENTE)
        conciliados       – cheques que estaban PENDIENTE (→ pasan al consolidado)
        actualizaciones   – [{row_index, estado_col}] para marcar CONCILIADO en Sheets
    """
    ...
    if not pendientes_raw:
        # Hoja vacía: todos los cheques son nuevos pendientes.
    ...
    if 'ESTADO' in first:
        ...
        # Estructura legacy de n8n: sin cabecera, identification en col 1, payment_amount en 9.
        data_rows   = pendientes_raw
        start_row   = 1
        ident_idx, valor_idx, estado_idx = 1, 9, 12

    # Mapa: "identification|valor_normalizado" → [{row_index, estado, estado_col}]
    mapa: dict[str, list] = {}
    for i, row in enumerate(data_rows, start=start_row):
        ident  = str(row[ident_idx]  if len(row) > ident_idx  else '').strip()
        valor  = _norm_valor_str(row[valor_idx]  if len(row) > valor_idx  else '')
    ...
    for row in cheques:
        clave = f'{row[0]}|{_norm_valor_str(row[8])}'  # identification|payment_amount
