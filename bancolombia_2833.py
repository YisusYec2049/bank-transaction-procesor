...
Diferencias respecto a la cuenta 2576:
  - payment_method = 'PREBANCOLOMBIA'
  - matching_key usa ref1 sin eliminar ceros iniciales
  - Lista de descripciones ajustada a este extracto
"""
...
def parse_pdf(pdf_bytes: io.BytesIO) -> list[dict]:
    """
    Extrae transacciones del extracto PDF de Bancolombia 2833.
...
def normalize(raw_rows: list[dict]) -> list[list]:
    """
    Convierte filas raw al esquema estándar.
...
def cheque_logic(
    ...
    for row in cheques:
        clave = f'{row[0]}|{_norm_valor_str(row[8])}'
